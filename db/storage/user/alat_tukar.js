...
//════════════════════════════//
  const fs = require('fs')
  let _inventoryOrang = JSON.parse(fs.readFileSync('./db/storage/user/inventory.json'))
  ...
  const addInventori = (sender) => {
        ...
        fs.writeFileSync('./db/storage/user/inventory.json', JSON.stringify(_inventoryOrang))
   }
