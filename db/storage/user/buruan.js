...
//════════════════════════════//
  const fs = require('fs')
  let _hasilBuruan = JSON.parse(fs.readFileSync('./db/storage/user/hasil_buruan.json'))
  
  const addInventoriBuruan = (sender) => {
        const obj = {id: sender, ikan: 0, ayam: 0, kelinci: 0, domba: 0, sapi: 0, gajah: 0}
         _hasilBuruan.push(obj)
        fs.writeFileSync('./db/storage/user/hasil_buruan.json', JSON.stringify(_hasilBuruan))
   }
